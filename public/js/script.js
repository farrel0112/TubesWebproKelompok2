...
if (emergencyForm && emergencyMessage && emergencyStatus) {
  emergencyForm.addEventListener("submit", function (e) {
    ...
    if (emergencyForm && emergencyMessage && emergencyStatus) {
      emergencyForm.addEventListener("submit", async function (e) {
        ...
        try {
          await apiRequest("/complaints/emergency", {
            method: "POST",
          ...
          console.error(err);
          emergencyStatus.textContent = "⚠️ Gagal mengirim panggilan. Coba lagi.";
          emergencyStatus.style.color = "red";
          emergencyStatus.style.display = "block";
        }
      });
